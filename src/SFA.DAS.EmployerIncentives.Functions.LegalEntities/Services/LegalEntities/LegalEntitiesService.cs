...
namespace SFA.DAS.EmployerIncentives.Functions.LegalEntities.Services.LegalEntities
{
    public class LegalEntitiesService : ILegalEntitiesService
    {
        ...
        {
            var response = await _client.PostAsJsonAsync($"{_client.BaseAddress.AbsolutePath.TrimEnd('/')}/accounts/{request.AccountId}/legalEntities", request);
            response.EnsureSuccessStatusCode();
        }

        public async Task Remove(RemoveRequest request)
        {
            var response = await _client.DeleteAsync($"{_client.BaseAddress.AbsolutePath.TrimEnd('/')}/accounts/{request.AccountId}/legalEntities/{request.AccountLegalEntityId}");
            response.EnsureSuccessStatusCode();
        }
    }
}
